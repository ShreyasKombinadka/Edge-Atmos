 ...
 */
void gpio_setup(uint8_t PIN, uint8_t PIN_PORT, uint8_t PIN_MODE, uint8_t PIN_CNF) // Set PIN configuration and mode
{
    switch (PIN_PORT)
    {
    case 'A': // GPIOA
        if (PIN < 16)
        {
            if (PIN < 8)
            {
                ...
                GPIOA->CRL |= ((((PIN_CNF << 2) & 12) | (PIN_MODE & 3)) << (PIN * 4)); // Set PIN configuration and mode
            }
            else
            {
                GPIOA->CRH &= ~(0xF << ((PIN - 8) * 4));                                     // Clear configuration
                GPIOA->CRH |= ((((PIN_CNF << 2) & 12) | (PIN_MODE & 3)) << ((PIN - 8) * 4)); // Set PIN configuration and mode
            }
        }
    ...
    case 'B': // GPIOB
        if (PIN < 16)
        {
            if (PIN < 8)
            {
                ...
                GPIOB->CRL |= ((((PIN_CNF << 2) & 12) | (PIN_MODE & 3)) << (PIN * 4)); // Set PIN configuration and mode
            }
            else
            {
                GPIOB->CRH &= ~(0xF << ((PIN - 8) * 4));                                     // Clear configuration
                GPIOB->CRH |= ((((PIN_CNF << 2) & 12) | (PIN_MODE & 3)) << ((PIN - 8) * 4)); // Set PIN configuration and mode
            }
        }
    ...
    case 'C': // GPIOC
        if (PIN < 16)
        {
            if (PIN < 8)
            {
                ...
                GPIOC->CRL |= ((((PIN_CNF << 2) & 12) | (PIN_MODE & 3)) << (PIN * 4)); // Set PIN configuration and mode
            }
            else
            {
                GPIOC->CRH &= ~(0xF << ((PIN - 8) * 4));                                     // Clear configuration
                GPIOC->CRH |= ((((PIN_CNF << 2) & 12) | (PIN_MODE & 3)) << ((PIN - 8) * 4)); // Set PIN configuration and mode
            }
        }
    ...
    case 'D': // GPIOD
        if (PIN < 16)
        {
            if (PIN < 8)
            {
                ...
                GPIOD->CRL |= ((((PIN_CNF << 2) & 12) | (PIN_MODE & 3)) << (PIN * 4)); // Set PIN configuration and mode
            }
            else
            {
                GPIOD->CRH &= ~(0xF << ((PIN - 8) * 4));                                     // Clear configuration
                GPIOD->CRH |= ((((PIN_CNF << 2) & 12) | (PIN_MODE & 3)) << ((PIN - 8) * 4)); // Set PIN configuration and mode
            }
        }
    ...
    case 'E': // GPIOE
        if (PIN < 16)
        {
            if (PIN < 8)
            {
                ...
                GPIOE->CRL |= ((((PIN_CNF << 2) & 12) | (PIN_MODE & 3)) << (PIN * 4)); // Set PIN configuration and mode
            }
            else
            {
                GPIOE->CRH &= ~(0xF << ((PIN - 8) * 4));                                     // Clear configuration
                GPIOE->CRH |= ((((PIN_CNF << 2) & 12) | (PIN_MODE & 3)) << ((PIN - 8) * 4)); // Set PIN configuration and mode
            }
        }
