...
static struct W25Q32 w25q32;

void w25q32_init(uint8_t SLAVE_CS, uint8_t SLAVE_CS_PIN_PORT) // Flash initialisation
{
    w25q32.SLAVE_CS = SLAVE_CS;
    w25q32.SLAVE_CS_PIN_PORT = SLAVE_CS_PIN_PORT;

    spi1_slaveset(w25q32.SLAVE_CS, w25q32.SLAVE_CS_PIN_PORT, 2);
}

void w25q32_read(uint32_t MEM_LOCATION_24, uint8_t *DATA, uint8_t DATA_SIZE_BYTE) // Flash memory read
{
    spi1_slaveselect(w25q32.SLAVE_CS, w25q32.SLAVE_CS_PIN_PORT, 1); // Select slave device
    ...
    spi1_slaveselect(w25q32.SLAVE_CS, w25q32.SLAVE_CS_PIN_PORT, 0); // De-select slave device
}
